...
export default function BusinessDashboard() {
  ...
  const menuItems = [
    { label: "Dashboard", icon: "📊", path: "/dashboard/business" },
    { label: "Live Tracking", icon: "🚛", path: "/dashboard/business/tracking" },
    { label: "Shipments", icon: "📦", path: "/dashboard/business/shipments" },
    { label: "Product Catalogue", icon: "📋", path: "/dashboard/business/products" },
    { label: "Fleet Management", icon: "🛠️", path: "/dashboard/business/fleet" },
    { label: "Settings", icon: "⚙️", path: "/dashboard/business/settings" },
  ];
            ...
            {menuItems.map((item, idx) => (
              <motion.div
                ...
                className={`flex items-center gap-4 px-6 py-4 rounded-3xl text-lg font-medium cursor-pointer transition-all ${
                  activePage === item.label ? "bg-[#004ac6] text-white" : "hover:bg-gray-100"
                }`}
              >
                <span>{item.icon}</span>
                {item.label}
